...
public class GlavnaController {
    ...
    public GlavnaController() {
        ...
        listaZnamenitosti = FXCollections.observableArrayList(dao.znamenitosti());
    }
    ...
    public void actionIzmijeniGrad(ActionEvent actionEvent) {
        ...
        if (grad == null) return;
        ArrayList<Znamenitosti> znamenitostiZaGrad = new ArrayList<>(dao.nadjiZnamenitosti(grad));
        listaZnamenitosti.setAll(znamenitostiZaGrad);

        Stage stage = new Stage();
        Parent root = null;
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/izmjeniGrad.fxml"));
            GradController gradController = new GradController(grad, dao.drzave(), listaZnamenitosti);
            loader.setController(gradController);
            root = loader.load();
            stage.setTitle("Izmjeni grad");
            stage.setScene(new Scene(root, USE_COMPUTED_SIZE, USE_COMPUTED_SIZE));
            stage.setResizable(true);
            stage.show();
    ...
    public void actionPrintajIzvjestaj(ActionEvent actionEvent) {
        try {
    ...
    public void actionOdabirJezika(ActionEvent actionEvent) throws IOException {
        Stage s = new Stage();

        String jezici[] = { "Bosanski", "Engleski" };

        ChoiceDialog d = new ChoiceDialog(jezici[1], jezici);

        Optional<String> text = d.showAndWait();
        if (text.get().equals("Engleski")) {
    ...
    public void refreshGlavna() throws IOException {
